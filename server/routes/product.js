...
router.get('/category/:category', async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Error obteniendo productos por categoría' });
    }
});
...
router.get('/admin/stats', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        const lowStockProducts = await Product.count({ where: { stock: { [Op.lt]: 10 }, active: true } });
        const categories = await Product.findAll({ attributes: ['category'], group: ['category'], raw: true });
        const totalCategories = categories.length;

        res.json({
            success: true,
            data: { totalProducts, featuredProducts, lowStockProducts, totalCategories }
        });
    } catch (error) {
    ...
    try {
        ...
        if (sizes) {
            if (typeof sizes === 'string') {
                try { productData.sizes = JSON.parse(sizes); } 
                catch { productData.sizes = [sizes]; }
            } else { productData.sizes = sizes; }
        } else { productData.sizes = []; }

        productData.image = req.file ? `/uploads/${req.file.filename}` : 'https://via.placeholder.com/300';

        const product = await Product.create(productData);
        res.status(201).json({ success: true, message: 'Producto creado correctamente', data: product });
    } catch (error) {
        console.error('Error creando producto:', error);
        res.status(500).json({ success: false, message: 'Error creando producto: ' + error.message });
    }
});
...
router.put('/admin/products/:id', authenticateToken, requireAdmin, upload.single('image'), async (req, res) => {
    try {
        ...
        if (sizes !== undefined) {
            if (typeof sizes === 'string') {
                try { updateData.sizes = JSON.parse(sizes); } 
                catch { updateData.sizes = [sizes]; }
            } else { updateData.sizes = sizes; }
        }
        ...
        res.status(500).json({ success: false, message: 'Error actualizando producto: ' + error.message });
    }
});
...
router.delete('/admin/products/:id', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Error eliminando producto: ' + error.message });
    }
});
